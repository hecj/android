...
public class ClientContext {
	...
	private static String EMCHAT_CONFIG = "emchat.properties";
	...
	private void initFromPropertiesFile() {
		...
		try {
			InputStream inputStream = ClientContext.class.getClassLoader().getResourceAsStream(EMCHAT_CONFIG);
			p.load(inputStream);
		} catch (IOException e) {
			log.error(MessageTemplate.print(MessageTemplate.FILE_ACCESS_MSG, new String[]{EMCHAT_CONFIG}));
		...
		if( StringUtils.isBlank(protocal) || StringUtils.isBlank(host) || StringUtils.isBlank(org) || StringUtils.isBlank(app) || StringUtils.isBlank(clientId) || StringUtils.isBlank(clientSecret) || StringUtils.isBlank(impLib) ) {
			log.error(MessageTemplate.print(MessageTemplate.INVAILID_PROPERTIES_MSG, new String[]{EMCHAT_CONFIG}));
			return; // Context not initialized
		}
	...
	public static void main(String[] args) {
		...
		System.out.println(getInstance().getClientId());
	}
