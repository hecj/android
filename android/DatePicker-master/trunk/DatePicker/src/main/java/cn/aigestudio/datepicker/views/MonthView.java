...
public class MonthView extends View {
    ...
    public void showDate(String date){
        ...
        DPInfo[][] dpInfos = mCManager.obtainDPInfo(centerYear, centerMonth);
        for (int i=0; i<dpInfos.length; i++){
            DPInfo[] dp2 = dpInfos[i];
            for (int j=0; j<dp2.length; j++){
                DPInfo dpInfo = dp2[j];
                if(dpInfo.strG.equals(date)){

                    Region[][] tmp;
                    if (TextUtils.isEmpty(dpInfos[4][0].strG)) {
                        tmp = MONTH_REGIONS_4;
                    } else if (TextUtils.isEmpty(dpInfos[5][0].strG)) {
                        tmp = MONTH_REGIONS_5;
                    } else {
                        tmp = MONTH_REGIONS_6;
                    }
                    ...
                    defineRegion(rect.centerX(),rect.centerY());
                    return;
                }
            }
        }
