/*! qding - v0.1.0 - * Copyright (c) 2015 lovemoon@yeah.net; Licensed GPLv2 */
// 定义Module需使用此方法 因为一个应用可能会打包多个模块
// 譬如将accountModule作为登录登出模块放置到其他模块中
// 注意：如果多模块整合可能会出现重名问题，后定义的服务会覆盖前面的

var __imgUrl__=""
parent.callback=function(url){
  __imgUrl__=url;
  $("#__logoImg__").attr("src",__imgHead__+url);
  alert('图片上传成功！');
}
var __env__="";
var __imgHead__="";
if(location.href.indexOf("dev")!=-1){
  __env__="dev";
  __imgHead__="http://dev.static.duomeidai.com/";
}else if(location.href.indexOf("test")!=-1){
  __env__="test";
  __imgHead__="http://fdfs.duomeidai.com/";
}else{
  __env__="pro";
  __imgHead__='http://fdfs.online.duomeidai.com/';
}
      ...
      response: function (response) {
        if (angular.isObject(response.data)) {
          ...
          if (response.config.$parsing !== false) {
            if ([0,"0", 200,"200"].indexOf(res.code) !== -1) {
              return res.data;
...
app.value('menubar', [{
    "id": 1000000000,
    "name": "小工具",
    "href": "/web/menus/broker#brokers/qrcode",
    "menus": [{
      "href": "/web/menus/broker#brokers/qrcode",
      "name": "生成二维码",
      "id": 1000000001
    ...
    "menus": [{
      ...
      "href": "/web/menus/broker#brokers/offline",
      "name": "地推商户",
      "id": 8000000003
    }]
  },{
...
app.directive('csLayout', function ($rootScope) {
  return {
    ...
    link: function (scope, element, attrs) {
      // 例外名单：不需要登陆 不适用默认布局
      var excludes = ['account.login', 'account.logout','brokers.login'];
  ...
  return function() {
    ...
    var proxy = function(name, params, data, method) {
      ...
      return $http({
        ...
        params: params || null,
        data: data || null,
        method: method ? method : ((name === 'create' || name === 'update') ? 'post' : 'get')
      });
    };
    ...
    var service = {
      ...
      create: function(data) {
        return proxy('create', data, null);
        //return proxy('create', null, data);
      },

      update: function(data) {
        return proxy('update', data, null);
...
app.factory('validater', function() {
  return {
    mobile: function(str) {
...
activityModule.controller('actlistController', ['$scope', '$state', '$stateParams', 'activityService',
	function($scope, $state, $stateParams, service) {
		$scope.search = function(keep) {
			...
			params["size"] = 10;
			params["activityName"] = $scope.activityName;
...
//活动新增／编辑
activityModule.controller('acteditController', ['$scope', '$state', '$stateParams', '$modal', 'activityService', 'msgService', 'util',
	function($scope, $state, $stateParams, $modal, service, msgService, util) {
		...
		$scope.editMainTitle = "添加";
		if (id != -1) { /*编辑活动*/
			$scope.editMainTitle = "编辑";
			$scope.isEdit = false;
			var params = {
				"id": id
			};
			service.activityDetail(params).then(function(res) {
				...
				$scope.activity = res;
				_entity = util.cloneObj(res);
				__imgUrl__ = res.logoUrl;
			})
		}
			...
			$modal.open({
				...
				controller: ['$scope', function(scope) {
					...
					scope.confirm = function() {
						...
						var getAward = function(award) {
							return award.split("_");
							// for (var i = 0, len = awardlist.length; i < len; i++) {
							// 	//console.log(awardlist[i].sourceID +","+ id);
							// 	if (awardlist[i].sourceID == id) {
							// 		return awardlist[i];
							// 	}
							// }
						}
						...
						if (scope.entity.document && scope.entity.document.length > 23) {
							msgService.messageBox("中奖内容的文案不能大于23个字符");
							return;
						}
						...
						if (typeof(index) == "number") {
							if (percent > 100) {
								...
								scope.entity.probability = _percent;
								return;
							}
						} else {
						...
						if (scope.entity.extraPrizeTypeLimit == 5) {
							if (!scope.entity.limitStartAt) {
								msgService.messageBox("请选择抽奖条件的开始时间！");
								return;
							}
							if (!scope.entity.limitEndAt) {
								msgService.messageBox("请选择抽奖条件的结束时间！");
								return;
							}
						}
						//console.log(scope.entity);
						if (typeof(index) != "number") {
							var _award = getAward(scope.entity.awardStr);
							scope.entity.awardName = _award[2];
							scope.entity.sourceID = _award[0];
							scope.entity.type = _award[1];
						}
		...
		$scope.save = function() {
			...
			$scope.activity.logoUrl = (__imgUrl__ == "") ? $scope.activity.logoUrl : __imgUrl__;
			$scope.activity.rules[0].exPrizeDrawStartAt = ($scope.activity.rules[0].exPrizeDrawStartAt) ? $scope.activity.rules[0].exPrizeDrawStartAt * 1 : 0;
			$scope.activity.rules[0].exPrizeDrawEndAt = ($scope.activity.rules[0].exPrizeDrawEndAt) ? service.getEndTime($scope.activity.rules[0].exPrizeDrawEndAt * 1) : 0;
			// $scope.activity.rules[0].exPrizeDrawEndAt = ($scope.activity.rules[0].exPrizeDrawEndAt) ? ($scope.activity.rules[0].exPrizeDrawEndAt * 1 + 24 * 3600000 - 1) : 0;
			$scope.activity.endAt = ($scope.activity.endAt) ? service.getEndTime($scope.activity.endAt * 1) : 0;
			$scope.activity.startAt = ($scope.activity.startAt) ? $scope.activity.startAt * 1 : 0;
		...
		$scope.deleteAward = function(index) {
			...
			if ($scope.started) {
				...
				$scope.activity.awards.splice(index, 1);
			}
		}
		$scope.quit = function() {
			if (id == -1) {
...
//中奖管理
activityModule.controller('awardlistController', ['$scope', '$state', '$stateParams', 'activityService', 'selectService', 'msgService', '$modal',
	function($scope, $state, $stateParams, service, selectService, msgService, $modal) {
		$scope.selected = false;
		$scope.page = 1;
		$scope.activityId = ($stateParams.id == -1) ? "" : $stateParams.id;
		$scope.search = function(keep) {
			var params = {};
			if (!keep) {
				$scope.page = 1;
		...
		var paramsActivity = {};
		paramsActivity["page"] = 1;
		paramsActivity["size"] = 100;
		service.activityList(paramsActivity).then(function(res) {
...
//商户管理
activityModule.controller('busilistController', ['$scope', '$state', '$stateParams', '$modal', 'activityService', 'selectService', 'msgService', 'util',
	function($scope, $state, $stateParams, $modal, service, selectService, msgService, util) {
		$scope.selected = false;
		$scope.page = 1;
		$scope.search = function(keep) {
			var params = {};
		...
		$scope.edit = function(item) {
			$modal.open({
				...
				controller: ['$scope', function(scope) {
					scope.entity = item ? util.cloneObj(item) : {};
					scope.type = item ? "编辑" : "新增";
					__imgUrl__ = (item) ? scope.entity.logoUrl : "";
					scope.updateImg = function(argument) {
						$("#imgFile").change(function() {
							$("#imgSubmit").click();
					...
					scope.confirm = function() {
						scope.entity.logoUrl = (__imgUrl__ == "") ? scope.entity.logoUrl : __imgUrl__;
...
//道具管理
activityModule.controller('itemlistController', ['$scope', '$state', '$stateParams', '$modal', 'activityService', 'selectService', 'msgService', 'util',
	function($scope, $state, $stateParams, $modal, service, selectService, msgService, util) {
		$scope.selected = false;
		$scope.page = 1;
		$scope.search = function(keep) {
		...
		$scope.edit = function(item) {
			$modal.open({
				...
				controller: ['$scope', function(scope) {
					...
					scope.entity = item ? util.cloneObj(item) : {};
					scope.type = item ? "编辑" : "新增";
					__imgUrl__ = (scope.entity) ? scope.entity.logoUrl : "";
...
//卡券管理
activityModule.controller('ticketlistController', ['$scope', '$state', '$stateParams', '$modal', 'activityService', 'selectService', 'msgService', 'util',
	function($scope, $state, $stateParams, $modal, service, selectService, msgService, util) {
		$scope.selected = false;
		$scope.page = 1;
		$scope.ticketType = 10;
		...
		$scope.edit = function(item) {
			$modal.open({
				...
				controller: ['$scope', function(scope) {
					...
					scope.type = item ? "编辑" : "新增";
					__imgUrl__ = (scope.entity.logoUrl) ? scope.entity.logoUrl : "";
					scope.dateArr = [{
						opened: false
					}, {
						opened: false
					...
					scope.confirm = function() {
						...
						if (!scope.entity.effectDays) {
							scope.entity.effectDays = 0;
						}
						if (!scope.entity.effectStartAt || !scope.entity.effectEndAt) {
							msgService.messageBox('请输入有效时间！');
							return;
						}
						if (!__imgUrl__) {
							msgService.messageBox('请上传卡券图片！');
							return;
						}
						scope.entity.effectStartAt = scope.entity.effectStartAt * 1;
						// scope.entity.effectEndAt = new Date(scope.entity.effectEndAt).toLocaleDateString() * 1 + 24 * 3600000 - 1;
						scope.entity.logoUrl = (__imgUrl__ == "") ? scope.entity.logoUrl : __imgUrl__;
						if (scope.entity.id) { /*编辑*/
							scope.entity.effectEndAt = scope.entity.effectEndAt * 1;
							...
							scope.entity.effectEndAt = scope.entity.effectEndAt * 1 + 24 * 3600000 - 1;
...
activityModule.filter("imgFilter",function(){
  return function (img) {
      if(__env__=='dev'){
        return 'http://dev.static.duomeidai.com/'+img;
      }
      else if(__env__=="test"){
        return 'http://fdfs.duomeidai.com/'+img;      
      }
      else{
        return 'http://fdfs.online.duomeidai.com/'+img;
      }
  };
})

activityModule.filter("ticketTypeFilter",function(){
...
activityModule.factory('util',function () {
  return{
...
angular.module('templates', ['config/templates/bootbox.partial.html', 'config/templates/checkbox.partial.html', 'config/templates/confirm.partial.html', 'config/templates/delete.partial.html', 'config/templates/layout.partial.html', 'config/templates/menubar.partial.html', 'config/templates/message.partial.html', 'config/templates/select.partial.html', 'modules/activity/templates/activity.edit.html', 'modules/activity/templates/activity.edit.rule.html', 'modules/activity/templates/activity.list.html', 'modules/activity/templates/award.list.html', 'modules/activity/templates/business.edit.html', 'modules/activity/templates/business.list.html', 'modules/activity/templates/item.edit.html', 'modules/activity/templates/item.list.html', 'modules/activity/templates/ticket.edit.html', 'modules/activity/templates/ticket.eidt.html', 'modules/activity/templates/ticket.list.html']);
...
angular.module("config/templates/layout.partial.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("config/templates/layout.partial.html",
    "<div ng-if=\"!$layout\" ui-view></div><div ng-if=\"$layout &amp;&amp; $user\"><header id=\"header\"><div class=\"container-fluid\"><a class=\"brand link-white pull-left\"><img ng-src=\"{{ $host }}/server/images/logo.png\" class=\"logo\" alt=\"多美贷\"> 多美贷</a><div class=\"settings pull-right\"><span class=\"dropdown\"><span class=\"text-right name mr10\" id=\"userNick\"></span> <a ui-sref=\"brokers.logout\" class=\"top-btn-logout\">退出</a></span></div></div></header><section id=\"content\"><aside class=\"sidebar\" cs-menubar></aside><div class=\"main\"><div growl></div><div ui-view></div></div></section><footer id=\"footer\"></footer></div><style type=\"text/css\">.dropdown-menu{z-index:2;}\n" +
    "  #content .main{min-height:500px;}</style>");
}]);
...
angular.module("config/templates/menubar.partial.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("config/templates/menubar.partial.html",
    "<ul class=\"menus list-unstyled\"><li ng-repeat=\"item in menubar\" ng-click=\"toggleMenu(item)\" ng-class=\"{active: item.active}\"><a href=\"\"><i class=\"fa fa-cog\"></i>{{ item.name }} <i class=\"fa pull-right\" ng-class=\"{'fa-angle-right': !item.active, 'fa-angle-down': item.active}\"></i></a><ul class=\"sub-menu list-unstyled\" ng-click=\"$event.stopPropagation()\"><li ng-repeat=\"menu in item.menus\" ui-sref-active=\"active\"><a ng-href=\"{{ menu.href }}\">{{ menu.name }}</a></li></ul></li></ul>");
}]);

angular.module("config/templates/message.partial.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("config/templates/message.partial.html",
    "<div class=\"modal-header\"><a class=\"close\" ng-click=\"$dismiss()\">&times;</a><h4 class=\"modal-title\">{{ title }}</h4></div><div class=\"modal-body\" ng-bind-html=\"message|safe\"></div><div class=\"modal-footer\"><button class=\"btn btn-default\" ng-click=\"$dismiss()\">确定</button></div>");
}]);
...
angular.module("modules/activity/templates/activity.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/activity.edit.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>{{editMainTitle}}活动</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list cjhdlist\"><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动名称：</label><div class=\"col-sm-10\"><input ng-disabled=\"!isEdit\" type=\"text\" ng-model=\"activity.name\" class=\"form-control mc-text-width\" placeholder=\"请输入活动名称\"></div></li><li style=\"overflow: inherit\"><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动时间：</label><div class=\"col-sm-10\"><input type=\"text\" ng-disabled=\"!isEdit||started\" class=\"form-control mc-auto-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"activity.startAt\" is-open=\"dateArr[0].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,0)\"> -- <input type=\"text\" ng-disabled=\"!isEdit||started\" class=\"form-control mc-auto-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"activity.endAt\" is-open=\"dateArr[1].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,1)\"></div></li><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动banner：</label><div class=\"col-sm-10\"><button ng-if=\"isEdit&&!activity.logoUrl\" ng-click=\"updateImg()\" type=\"button\" class=\"btn btn-default\">+添加活动banner</button></div><img ng-if=\"activity.logoUrl\" height=\"50\" ng-click=\"updateImg()\" ng-src=\"{{activity.logoUrl|imgFilter}}\" id=\"__logoImg__\"> <img ng-if=\"!activity.logoUrl\" height=\"50\" id=\"__logoImg__\"></li><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动介绍：</label><div class=\"col-sm-10\"><textarea ng-disabled=\"!isEdit\" class=\"form-control mc-auto-width\" ng-model=\"activity.description\" rows=\"5\" cols=\"80\" placeholder=\"\"></textarea></div></li><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动声明：</label><div class=\"col-sm-10\"><textarea ng-disabled=\"!isEdit\" class=\"form-control mc-auto-width\" ng-model=\"activity.declareTo\" rows=\"5\" cols=\"80\"></textarea></div></li><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>抽奖次数设置：</label><div class=\"col-sm-10\"><i>用户</i><select ng-disabled=\"!isEdit||started\" class=\"form-control mc-auto-width\" ng-model=\"activity.rules[0].basePrizeDrawType\"><option value=\"1\">每天</option><option value=\"2\">共</option><option value=\"3\">注册或实名认证</option></select><i>可抽奖</i> <input ng-disabled=\"!isEdit||started\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" ng-model=\"activity.rules[0].basePrizeDrawTime\" class=\"form-control mc-auto-width\">次</div></li><li style=\"overflow: inherit\"><label class=\"text-normal text-nowrap col-lg-2\">额外抽奖次数设置：</label><div class=\"col-sm-10\"><p>当用户满足下列条件可设置增加抽奖次数</p><div class=\"clearfix\"><p><input type=\"text\" ng-disabled=\"!isEdit||started\" class=\"form-control mc-auto-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"activity.rules[0].exPrizeDrawStartAt\" is-open=\"dateArr[2].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,2)\"> -- <input type=\"text\" ng-disabled=\"!isEdit||started\" class=\"form-control mc-auto-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"activity.rules[0].exPrizeDrawEndAt\" is-open=\"dateArr[3].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,3)\"></p><p><select ng-disabled=\"!isEdit||started\" ng-model=\"activity.rules[0].exPrizeDrawType\" class=\"form-control mc-auto-width\"><option value=\"\">请选择条件</option><option value=\"1\">每投资满</option><option value=\"2\">累计投资金额满</option><option value=\"3\">单笔投资金额满</option><option value=\"4\">每日投满</option></select><input ng-disabled=\"!isEdit||started\" type=\"text\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" ng-model=\"activity.rules[0].exPrizeDrawAmount\" class=\"form-control mc-auto-width\" placeholder=\"请输入投资金额\">元 <input ng-disabled=\"!isEdit||started\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" ng-model=\"activity.rules[0].exPrizeDrawTime\" class=\"form-control mc-auto-width\">次</p></div></div></li><li><label class=\"text-normal text-nowrap col-sm-2\"><i class=\"corred\">*</i>活动奖励及规则：</label><div class=\"col-sm-10\"><p><button type=\"button\" ng-if=\"isEdit\" ng-click=\"editAward()\" class=\"btn btn-default\">+添加活动奖品及抽奖规则</button></p><table class=\"table table-hover\" ng-if=\"activity.awards.length>0\"><thead><tr><th>奖品名称</th><th width=\"25%\">抽奖条件</th><th>奖品数量</th><th>中奖概率</th><th>奖品投放日期</th><th width=\"20%\">中奖文案</th><th ng-if=\"isEdit\" class=\"text-right\">操作</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in activity.awards\"><td class=\"ng-binding\">{{item.awardName}}</td><td class=\"ng-binding\">{{item.awardRuleText}}</td><td class=\"ng-binding\">{{item.count}}</td><td class=\"ng-binding\">{{item.probability}}%</td><td class=\"ng-binding\">{{item.releaseAt|date:\"yyyy-MM-dd\"}}</td><td class=\"ng-binding\">{{item.document}}</td><td class=\"text-right\" ng-if=\"isEdit\"><a ng-click=\"editAward($index)\">编辑</a> <i class=\"text-divider\">|</i> <a ng-click=\"deleteAward($index)\">删除</a></td></tr></tbody></table></div></li><li ng-if=\"isEdit\"><label class=\"text-normal text-nowrap col-sm-2\"></label><div class=\"col-sm-10\"><button class=\"btn btn-primary ml10\" style=\"display:none\" ng-click=\"preview()\" type=\"submit\"><i class=\"fa fa-search\"></i> 预览网页</button> <button class=\"btn btn-primary ml10\" ng-click=\"save()\"><i class=\"fa fa-search\"></i> 完成</button> <button class=\"btn btn-primary ml10\" ng-click=\"quit()\"><i class=\"fa fa-search\"></i> 返回</button></div></li><li ng-if=\"!isEdit\"><label class=\"text-normal text-nowrap col-sm-2\"></label><div class=\"col-sm-10\"><button class=\"btn btn-primary ml10\" ng-click=\"edit()\"><i class=\"fa fa-search\"></i> 编辑活动</button> <button class=\"btn btn-primary ml10\" ng-click=\"back()\"><i class=\"fa fa-search\"></i> 返回</button></div></li></ul></div></div></div></div><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);

angular.module("modules/activity/templates/activity.edit.rule.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/activity.edit.rule.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">{{editTitle}}奖品及抽奖规则</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i> 选择奖品：</label><select ng-model=\"entity.awardStr\" class=\"form-control mc-auto-width\" ng-disabled=\"!ruleEdit\"><option>请选择奖品</option><option ng-repeat=\"item in awardlist\" ng-selected=\"entity.sourceID==item.sourceID\" value=\"{{item.sourceID+'_'+item.type+'_'+item.name}}\">{{item.name}}</option></select></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i> 抽奖条件：</label><select ng-model=\"entity.extraPrizeTypeLimit\" class=\"form-control mc-auto-width\"><option value=\"1\" selected=\"selected\">注册用户</option><option value=\"2\">实名认证用户</option><option value=\"3\">有投资记录用户</option><option value=\"4\">累计投资金额满</option><option value=\"5\">单笔投资金额满</option></select><input type=\"text\" class=\"form-control mc-time-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"entity.limitStartAt\" is-open=\"dateArr[0].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,0)\"> <span style=\"float:left\">--</span> <input type=\"text\" class=\"form-control mc-time-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"entity.limitEndAt\" is-open=\"dateArr[1].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,1)\"> <input ng-if=\"entity.extraPrizeTypeLimit==4||entity.extraPrizeTypeLimit==5\" type=\"text\" ng-model=\"entity.limitInvestAmount\" class=\"form-control mc-time-width\" placeholder=\"请输入投资金额\"> <span ng-if=\"entity.extraPrizeTypeLimit==4||entity.extraPrizeTypeLimit==5\" style=\"float:left\">元</span></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i> 奖品数量：</label><input type=\"text\" ng-model=\"entity.count\" onkeypress=\"if (event.keyCode < 45 || event.keyCode >\n" +
    "    57) event.returnValue = false\" class=\"form-control mc-auto-width\" placeholder=\"请输入奖品数量\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i> 中奖概率：</label><input type=\"text\" ng-model=\"entity.probability\" class=\"form-control mc-auto-width\" placeholder=\"请输入中奖概率\"> <span style=\"float:left\">%</span></li><li><label class=\"text-normal text-nowrap\">奖品投放日期：</label><input type=\"text\" class=\"form-control mc-auto-width\" datepicker-popup=\"yyyy-MM-dd\" ng-model=\"entity.releaseAt\" is-open=\"dateArr[2].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,2)\"> <span style=\"float:left;font-size:12px\">奖品从投放日期开始，每天平均投放，若不填则从活动上线时刻开始。</span></li><li><label class=\"text-normal text-nowrap\">中奖文案：</label><input type=\"text\" ng-model=\"entity.document\" class=\"form-control mc-text-width\" placeholder=\"请输入中奖文案\"></li><li><label class=\"text-normal text-nowrap\">中奖短信模板：</label><input type=\"text\" ng-model=\"entity.winningSmsTemplate\" class=\"form-control mc-text-width\" placeholder=\"中奖短信模板\"></li><li><label class=\"text-normal text-nowrap\">中奖站内信模板：</label><input type=\"text\" ng-model=\"entity.winningLetterTemplate\" class=\"form-control mc-text-width\" placeholder=\"中奖站内信模板\"></li></ul></div><div class=\"modal-footer ng-scope wind_t_pub add_jp\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div>");
}]);

angular.module("modules/activity/templates/activity.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/activity.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>活动列表</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">活动名称：</label><input type=\"text\" ng-model=\"activityName\" class=\"form-control mc-auto-width\" placeholder=\"请输入活动名称\"></li><li><button class=\"btn btn-primary ml10\" ng-click=\"search()\"><i class=\"fa fa-search\"></i> 查找</button> <button ui-sref=\"activity.actedit({id: -1})\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 创建活动</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th>序号</th><th>活动名称</th><th>创建时间</th><th>活动开始时间</th><th>活动结束时间</th><th>中奖管理</th><th>状态</th><th class=\"text-right\">操作</th></tr></thead><tbody><tr ng-if=\"list.length!=0\" class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\"><a ui-sref=\"activity.actedit({id:item.id})\">{{item.activityName}}</a></td><td class=\"ng-binding\">{{item.createAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\">{{item.startAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\">{{item.endAt|date:'yyyy-MM-dd HH:mm'}}</td><td class=\"ng-binding\"><a ui-sref=\"activity.awardlist({id:item.id})\">中奖管理</a></td><td class=\"ng-binding\">{{item.status|activityStatus}}</td><td class=\"text-right\"><a ng-click=\"update(item)\">{{item.status|actStatus}}</a></td></tr><tr ng-if=\"list.length==0\"><td colspan=\"8\" style=\"text-align: center\">暂无数据</td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);

angular.module("modules/activity/templates/award.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/award.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>中奖管理</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap first-label\">活动名称：</label><select ng-model=\"activityId\" class=\"form-control mc-auto-width\" ng-change=\"getAwardSel()\"><option ng-repeat=\"item in activitySel\" value=\"{{item.id}}\">{{item.activityName}}</option></select></li><li><label class=\"text-normal text-nowrap\">奖品名称：</label><select ng-model=\"awardId\" class=\"form-control mc-auto-width\"><option value=\"\">所有奖品</option><option ng-repeat=\"item in awardlist\" value=\"{{item.id}}\">{{item.name}}</option></select></li><li><label class=\"text-normal text-nowrap\">奖品发放状态：</label><select ng-model=\"checked\" class=\"form-control mc-auto-width\"><option value=\"\">请选择奖品发放状态</option><option value=\"1\">已发放</option><option value=\"2\">未发放</option></select></li></ul></div></div><div class=\"row\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap first-label\">中奖用户手机号：</label><input type=\"text\" ng-model=\"mobile\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-auto-width\" placeholder=\"请输入用户手机号\"></li><li><button class=\"btn btn-primary ml10\" type=\"submit\" ng-click=\"search()\"><i class=\"fa fa-search\"></i> 查找</button> <button class=\"btn btn-primary ml10\" ng-click=\"markAward()\"><i class=\"fa fa-search\"></i> 发放奖品</button> <button class=\"btn btn-primary ml10\" ng-click=\"excel()\"><i class=\"fa fa-search\"></i> 导出EXCEL</button></li></ul></div></div><div class=\"row\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap first-label\">手动派奖：</label><select ng-model=\"_activityId\" class=\"form-control mc-auto-width\" ng-change=\"_getAwardSel()\"><option ng-repeat=\"item in activitySel\" value=\"{{item.id}}\">{{item.activityName}}</option></select></li><li><input type=\"text\" ng-model=\"_mobile\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-auto-width\" placeholder=\"请输入用户手机号\"></li><li><label class=\"text-normal text-nowrap\">奖品名称：</label><select ng-model=\"_awardId\" class=\"form-control mc-auto-width\"><option ng-repeat=\"item in _awardlist\" value=\"{{item.id}}\">{{item.name}}</option></select></li><li><button class=\"btn btn-primary ml10\" ng-click=\"giveAward()\"><i class=\"fa fa-search\"></i> 派奖</button></li></ul></div></div></div><table class=\"table table-hover\"><thead><tr><th><input type=\"checkbox\" ng-model=\"selected\" ng-change=\"selectAll()\" value=\"\">全选</th><th>序号</th><th>活动名称</th><th>奖品</th><th>中奖用户手机号</th><th>中奖时间</th><th>发放状态</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\"><input type=\"checkbox\" ng-model=\"item.selected\"></td><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\">{{item.activityName}}</td><td class=\"ng-binding\">{{item.awardName}}</td><td class=\"ng-binding\">{{item.winnerMobile}}</td><td class=\"ng-binding\">{{item.winningTimeAt|date:'yyyy-MM-dd'}}</td><td class=\"ng-binding\">{{item.checked|awardFilter}}</td></tr></tbody></table><div ng-show=\"total\" class=\"text-right\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);

angular.module("modules/activity/templates/business.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/business.edit.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">{{type}}商家</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>商家名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-name-width\" value=\"请输入商家名称\"></li><li><label class=\"text-normal text-nowrap\">商家网址：</label><input type=\"text\" ng-model=\"entity.domain\" class=\"form-control mc-name-width\" value=\"请输入商家网址\"></li><li><label class=\"text-normal text-nowrap\">商家地址：</label><input type=\"text\" ng-model=\"entity.address\" class=\"form-control mc-name-width\" value=\"请输入商家地址\"></li><li><label class=\"text-normal text-nowrap\">商家电话：</label><input type=\"text\" ng-model=\"entity.tel\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-name-width\" value=\"请输入商家电话\"></li><li><label class=\"text-normal text-nowrap\">商家传真：</label><input type=\"text\" ng-model=\"entity.fax\" class=\"form-control mc-name-width\" value=\"请输入商家传真\"></li><li><label class=\"text-normal text-nowrap\">商家微博：</label><input type=\"text\" ng-model=\"entity.weibo\" class=\"form-control mc-name-width\" value=\"请输入商家官方微博地址\"></li><li><label class=\"text-normal text-nowrap\">公众号：</label><input type=\"text\" ng-model=\"entity.wechat\" class=\"form-control mc-name-width\" value=\"请输入商家微信公众号\"></li><li><label class=\"text-normal text-nowrap\">邮箱：</label><input type=\"text\" ng-model=\"entity.mail\" class=\"form-control mc-name-width\" value=\"请输入商家邮箱\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>商家介绍：</label><textarea cols=\"80\" ng-model=\"entity.description\" rows=\"5\" class=\"form-control mc-name-width\">请输入商家介绍</textarea></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>商家logo：</label><button ng-if=\"!entity.logoUrl\" class=\"btn btn-primary\" ng-click=\"updateImg()\" ng-disabled=\"processing\">添加图片</button> <img ng-if=\"entity.logoUrl\" height=\"50\" ng-click=\"updateImg()\" ng-src=\"{{entity.logoUrl|imgFilter}}\" id=\"__logoImg__\"> <img ng-if=\"!entity.logoUrl\" height=\"50\" id=\"__logoImg__\"></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);

angular.module("modules/activity/templates/business.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/business.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>商家管理</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">关键字：</label><input type=\"text\" ng-model=\"name\" class=\"form-control mc-text-width\" placeholder=\"请输入商家名称\"></li><li><button ng-click=\"search(true)\" class=\"btn btn-primary ml10\" type=\"submit\"><i class=\"fa fa-search\"></i> 查找</button> <button ng-click=\"delete()\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 批量删除</button> <button ng-click=\"edit()\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 新增商家</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th><input type=\"checkbox\" ng-model=\"selected\" ng-change=\"selectAll()\" value=\"\">全选</th><th>序号</th><th>商家名称</th><th>logo</th><th width=\"30%\">商家介绍</th><th>操作</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\"><input type=\"checkbox\" ng-model=\"item.selected\"></td><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\">{{item.name}}</td><td class=\"ng-binding\"><img height=\"40\" ng-if=\"item.logoUrl!=''\" ng-src=\"{{item.logoUrl|imgFilter}}\"></td><td class=\"ng-binding\">{{item.description}}</td><td class=\"ng-binding\"><a ng-click=\"edit(item)\">编辑</a> <i class=\"text-divider\">|</i> <a ng-click=\"delete(item)\">删除</a></td></tr></tbody></table><div ng-show=\"total\" class=\"text-right\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);

angular.module("modules/activity/templates/item.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/item.edit.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">{{type}}道具</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>道具名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-auto-width\" value=\"请输入道具名称\"></li><li><label class=\"text-normal text-nowrap\">金额：</label><input type=\"text\" ng-model=\"entity.amount\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-auto-width\" value=\"请输入道具金额\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>选择商家：</label><select class=\"form-control mc-auto-width\" ng-model=\"entity.partnerID\"><option value=\"\">请选商家名称</option><option ng-repeat=\"item in businessList\" ng-selected=\"entity.partnerID==item.id\" value=\"{{item.id}}\">{{item.name}}</option></select></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>道具图片：</label><button ng-if=\"!entity.logoUrl\" class=\"btn btn-primary\" ng-click=\"updateImg()\" ng-disabled=\"processing\">添加图片</button> <img ng-if=\"entity.logoUrl\" height=\"50\" ng-click=\"updateImg()\" ng-src=\"{{entity.logoUrl|imgFilter}}\" id=\"__logoImg__\"> <img ng-if=\"!entity.logoUrl\" height=\"50\" id=\"__logoImg__\"></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);

angular.module("modules/activity/templates/item.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/item.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>道具管理</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">关键字：</label><input type=\"text\" ng-model=\"key\" class=\"form-control mc-text-width\" placeholder=\"请输入道具名称或者商家名称\"></li><li><button ng-click=\"search()\" class=\"btn btn-primary ml10\" type=\"submit\"><i class=\"fa fa-search\"></i> 查找</button> <button ng-click=\"edit()\" class=\"btn btn-primary ml10\"><i class=\"fa fa-search\"></i> 新增道具</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th><input type=\"checkbox\" ng-model=\"selected\" ng-change=\"selectAll()\" value=\"\">全选</th><th>序号</th><th>名称</th><th>金额(元)</th><th width=\"30%\">商家名称</th><th>操作</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\"><input type=\"checkbox\" ng-model=\"item.selected\"></td><td class=\"ng-binding\">{{$index+1+(page-1)*(size*1)}}</td><td class=\"ng-binding\">{{item.name}}</td><td class=\"ng-binding\">{{item.amount}}</td><td class=\"ng-binding\">{{item.partnerID}}</td><td class=\"ng-binding\"><a ng-click=\"edit(item)\">编辑</a> <i class=\"text-divider\"></i> <a ng-click=\"delete(item)\" style=\"display: none\">删除</a></td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);

angular.module("modules/activity/templates/ticket.edit.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/ticket.edit.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">{{type}}卡券</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券类型：</label><label class=\"radio-inline\"><input style=\"margin-top:13px\" type=\"radio\" name=\"inlineRadioOptions\" ng-model=\"entity.type\" value=\"10\">返利券</label></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-auto-width\" value=\"请输入卡券名称\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>金额：</label><input type=\"text\" ng-model=\"entity.value\" onkeypress=\"if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false\" class=\"form-control mc-auto-width\" value=\"请输入卡券金额\"></li><li><label class=\"text-normal text-nowrap\">返利有效期：</label><input type=\"text\" class=\"form-control mc-auto-width\" ng-model=\"entity.effectDays\" value=\"请输入返利使用有效期\"><span>天</span><span style=\"font-size:12px;color:#999;margin-left:10px\">若不填，则时间不做限制</span></li><li><label class=\"text-normal text-nowrap\">返利有效期：</label><input type=\"text\" class=\"form-control mc-time-width\" datepicker-popup=\"yyyy-MM-dd\" placeholder=\"开始日期\" ng-model=\"entity.effectStartAt\" is-open=\"dateArr[0].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,0)\"> <span style=\"float:left\">--</span> <input type=\"text\" class=\"form-control mc-time-width\" datepicker-popup=\"yyyy-MM-dd\" placeholder=\"结束日期\" ng-model=\"entity.effectEndAt\" is-open=\"dateArr[1].opened\" close-text=\"Close\" ng-click=\"dateOpen($event,1)\"></li><li><label>卡券图片：</label><button ng-if=\"!entity.logoUrl\" class=\"btn btn-primary\" ng-click=\"updateImg()\" ng-disabled=\"processing\">添加图片</button> <img ng-if=\"entity.logoUrl\" height=\"50\" ng-click=\"updateImg()\" ng-src=\"{{entity.logoUrl|imgFilter}}\" id=\"__logoImg__\"> <img ng-if=\"!entity.logoUrl\" height=\"50\" id=\"__logoImg__\"></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);

angular.module("modules/activity/templates/ticket.eidt.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/ticket.eidt.html",
    "<div class=\"modal-header ng-scope wind_t_pub add_jp\"><a class=\"close\" ng-click=\"$dismiss()\">×</a><h4 class=\"modal-title ng-binding\">新增卡券</h4></div><div class=\"modal-body ng-scope wind_t_pub add_jp\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券类型：</label><label class=\"radio-inline\"><input type=\"radio\" name=\"inlineRadioOptions\" value=\"\">返利券</label><label class=\"radio-inline\"><input type=\"radio\" name=\"inlineRadioOptions\" value=\"\">再来一次券</label></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券名称：</label><input type=\"text\" ng-model=\"entity.name\" class=\"form-control mc-auto-width\" value=\"请输入卡券名称\"></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>金额：</label><input type=\"text\" ng-model=\"entity.money\" class=\"form-control mc-auto-width\" value=\"请输入卡券金额\"> <span class=\"corred\">注：当选择【再来一次券】时，金额非必填</span></li><li><label class=\"text-normal text-nowrap\">返利有效期：</label><input type=\"text\" ng-model=\"entity.deadline\" class=\"form-control mc-auto-width\" value=\"请输入返利使用有效期\"><span>天</span><span style=\"font-size:12px;color:#999;margin-left:10px\">若不填，则时间不做限制</span></li><li><label class=\"text-normal text-nowrap\"><i class=\"corred\">*</i>卡券图片：</label><button class=\"btn btn-primary\" ng-click=\"updateImg()\" ng-disabled=\"processing\">添加图片</button> <img ng-if=\"entity.logoUrl\" ng-src=\"entity.logoUrl\" id=\"__logoImg__\"> <img ng-if=\"!entity.logoUrl\" id=\"__logoImg__\"></li></ul></div><div class=\"modal-footer ng-scope\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"$dismiss()\">取消</button></div><form style=\"display:none\" action=\"/upload\" method=\"post\" enctype=\"multipart/form-data\" target=\"_uploadIFrame\"><input type=\"file\" name=\"file\" id=\"imgFile\"> <input type=\"submit\" value=\"upload\" id=\"imgSubmit\"></form><iframe name=\"_uploadIFrame\" style=\"display:none\"></iframe>");
}]);

angular.module("modules/activity/templates/ticket.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/activity/templates/ticket.list.html",
    "<div class=\"account-module ng-scope\"><div class=\"page-header\"><h1>卡券管理</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-11\"><div class=\"form-inline\"><ul class=\"se-list\"><li><label class=\"text-normal text-nowrap\">卡券类型：</label><select class=\"form-control\" ng-model=\"type\"><option value=\"\" ng-selected=\"true\">所有卡券</option><option value=\"10\">返利券</option></select></li><li><label class=\"text-normal text-nowrap\">关键字：</label><input type=\"text\" ng-model=\"key\" class=\"form-control mc-text-width\" placeholder=\"请输入卡券名称或者金额\"></li><li><button class=\"btn btn-primary ml10\" ng-click=\"search()\" type=\"submit\"><i class=\"fa fa-search\"></i> 查找</button> <button class=\"btn btn-primary ml10\" ng-click=\"edit()\"><i class=\"fa fa-search\"></i> 新增卡券</button></li></ul></div></div></div></div><table class=\"table table-hover\"><thead><tr><th><input type=\"checkbox\" ng-model=\"selected\" ng-change=\"selectAll()\" value=\"\">全选</th><th>序号</th><th>卡券类型</th><th width=\"30%\">名称</th><th>金额（元）</th><th>操作</th></tr></thead><tbody><tr class=\"ng-scope\" ng-repeat=\"item in list\"><td class=\"ng-binding\"><input type=\"checkbox\" ng-model=\"item.selected\"></td><td class=\"ng-binding\">{{$index+1}}</td><td class=\"ng-binding\">{{item.type|ticketTypeFilter}}</td><td class=\"ng-binding\">{{item.name}}</td><td class=\"ng-binding\">{{item.value}}</td><td class=\"ng-binding\"><a ng-click=\"edit(item)\">编辑</a> <i class=\"text-divider\"></i> <a ng-click=\"delete(item)\" style=\"display: none\">删除</a></td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search(true)\"></ul></div></div>");
}]);
