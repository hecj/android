...
// define module
var redModule = Modules.register('redModule', ['app']);

// config routes
redModule.config(function($stateProvider, $urlRouterProvider) { 

  $urlRouterProvider.otherwise("/red/login");
  ...
  $stateProvider
    ...
    .state('red.login', {
      url: '/login',
      templateUrl: "modules/red/templates/login.html",
      controller: 'loginController'
    })
    .state('red.list', {
      url: '/list',
      templateUrl: "modules/red/templates/list.html",
      controller: 'listController'
    });
});
redModule.controller('loginController', ['$scope','$state','redService',function ($scope, $state, service) {
...
redModule.controller('listController', ['$scope', '$state','$module', 'clientService',
	function($scope, $state,$module, service) {
...
redModule.factory('redService', function($q, $http, serviceGenerator) {
...
angular.module('templates', ['config/templates/bootbox.partial.html', 'config/templates/checkbox.partial.html', 'config/templates/confirm.partial.html', 'config/templates/delete.partial.html', 'config/templates/layout.partial.html', 'config/templates/menubar.partial.html', 'config/templates/message.partial.html', 'config/templates/select.partial.html', 'modules/red/templates/change.html', 'modules/red/templates/invest.html', 'modules/red/templates/list.html', 'modules/red/templates/login.html']);
...
angular.module("modules/red/templates/change.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/red/templates/change.html",
    "<div class=\"modal-header\"><a class=\"close\" ng-click=\"$dismiss()\">&times;</a><h4 class=\"modal-title\">{{ title }}</h4></div><div class=\"modal-body\"><label class=\"control-label text-normal text-nowrap\" style=\"width:50px;float:left;line-height:30px\">机构名：</label><input type=\"text\" class=\"form-control\" id=\"add-phone\" ng-model=\"name\" ng-pattern=\"/^[1][2-9]\\d{9}$/\" style=\"width:120px;float:left\"><label class=\"control-label text-normal text-nowrap\" style=\"width:50px;float:left;line-height:30px\">详情：</label><input type=\"file\" class=\"form-control\" id=\"add-phone\" ng-model=\"detail\" ng-pattern=\"/^[1][2-9]\\d{9}$/\" style=\"width:120px;float:left\"><label class=\"control-label text-normal text-nowrap\" style=\"width:50px;float:left;line-height:30px\">logo：</label><input type=\"file\" class=\"form-control\" id=\"add-phone\" ng-model=\"logo\" ng-pattern=\"/^[1][2-9]\\d{9}$/\" style=\"width:120px;float:left\"><label class=\"control-label text-normal text-nowrap\" style=\"width:50px;float:left;line-height:30px\">图片：</label><input type=\"file\" class=\"form-control\" id=\"add-phone\" ng-model=\"pic\" ng-pattern=\"/^[1][2-9]\\d{9}$/\" style=\"width:120px;float:left\"><label class=\"control-label text-normal text-nowrap\" style=\"width:50px;float:left;line-height:30px\">说：</label><input type=\"file\" class=\"form-control\" id=\"add-phone\" ng-model=\"say\" ng-pattern=\"/^[1][2-9]\\d{9}$/\" style=\"width:120px;float:left\"></div><div class=\"modal-footer\"><button class=\"btn btn-primary\" ng-click=\"confirm()\" ng-disabled=\"processing\">确定</button> <button class=\"btn btn-default\" ng-click=\"close()\">取消</button></div>");
}]);

angular.module("modules/red/templates/invest.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/red/templates/invest.html",
    "<div class=\"account-module\"><div class=\"page-header\"><h1>第三方网站</h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-12\"><div class=\"userInfo\"><ul class=\"se-list\"><li><label class=\"control-label text-normal text-nowrap txtleft\">姓名：</label>{{userInfo.realName}}</li><li><label class=\"control-label text-normal text-nowrap txtleft\">昵称：</label>{{userInfo.userName}}</li><li><label class=\"control-label text-normal text-nowrap txtleft\">ID：</label>{{userInfo.tuserId}}</li><li><label class=\"control-label text-normal text-nowrap txtleft\">手机号：</label>{{userInfo.mobilePhone}}</li></ul><p><label class=\"control-label text-normal text-nowrap txtleft\">经纪人：</label>{{userInfo.investerName|brokerFilter}}</p><button ng-if=\"!userInfo.investerName\" type=\"submit\" class=\"btn btn-primary ml10\" ng-disabled=\"form.$invalid || processing\" ng-click=\"edit('add')\"><i class=\"fa fa-search\"></i> 添加</button> <button ng-if=\"userInfo.investerName\" type=\"submit\" class=\"btn btn-primary ml10\" ng-disabled=\"form.$invalid || processing\" ng-click=\"edit('update')\"><i class=\"fa fa-search\"></i> 修改</button> <button ng-if=\"userInfo.investerName\" type=\"submit\" class=\"btn btn-primary ml10\" ng-disabled=\"form.$invalid || processing\" ng-click=\"delete()\"><i class=\"fa fa-search\"></i> 删除</button></div><div class=\"form-inline\"><ul class=\"se-list\"><li><button type=\"submit\" class=\"btn btn-primary ml10\" ng-disabled=\"form.$invalid || processing\" ng-click=\"tabChange(1)\"><i class=\"fa fa-search\"></i> 投资记录</button></li><li><button type=\"submit\" class=\"btn btn-primary ml10\" ng-disabled=\"form.$invalid || processing\" ng-click=\"tabChange(2)\"><i class=\"fa fa-search\"></i> 变更记录</button></li></ul></div></div></div></div><table class=\"table table-hover\" ng-if=\"detailType==2\"><thead><tr><th>投资时间</th><th>投资项目</th><th>项目ID</th><th>投资期限(月)</th><th>年化利率</th><th>经纪人</th><th>投资金额</th></tr></thead><tbody><tr ng-if=\"loading\"><td class=\"loading-holder\" colspan=\"40\"><div class=\"text-center\"><span class=\"loading\"></span></div></td></tr><tr ng-if=\"list.length == 0\"><td colspan=\"40\"><div class=\"text-center\">抱歉，没有相关数据</div></td></tr><tr ng-repeat=\"item in list\" ng-if=\"!loading\"><td>{{ item.investTime|date:'yyyy-MM-dd HH:mm' }}</td><td>{{ item.borrowTitle }}</td><td>{{ item.borrowId }}</td><td>{{ item.deadline }}</td><td>{{ item.annualRate }}</td><td>{{ item.investAmount }}</td><td>{{ item.brokerType|brokerType }}</td></tr></tbody></table><table class=\"table table-hover\" ng-if=\"detailType==2\"><thead><tr><th>操作人</th><th>操作时间</th><th>原经纪人</th><th>现经纪人</th></tr></thead><tbody><tr ng-if=\"loading\"><td class=\"loading-holder\" colspan=\"40\"><div class=\"text-center\"><span class=\"loading\"></span></div></td></tr><tr ng-if=\"list.length == 0\"><td colspan=\"40\"><div class=\"text-center\">抱歉，没有相关数据</div></td></tr><tr ng-repeat=\"item in list\" ng-if=\"!loading\"><td>{{ item.operator }}</td><td>{{ item.operateTime|date:'yyyy-MM-dd HH:mm' }}</td><td>{{ item.content }}</td><td>{{ item.moduletype }}</td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search()\"></ul></div></div>");
}]);

angular.module("modules/red/templates/list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/red/templates/list.html",
    "<div class=\"account-module\"><div class=\"page-header\"><h1></h1></div><div class=\"filter clearfix\"><div class=\"row\"><div class=\"col-sm-12\"><button class=\"btn btn-primary ml10\" ng-click=\"add()\"><i class=\"fa fa-search\"></i> 添加</button></div></div></div><table class=\"table table-hover\"><thead><tr><th>ID</th><th>名称</th><th>详情</th><th>logo</th><th>图片</th><th>说</th><th>链接</th><th class=\"text-right\">操作</th></tr></thead><tbody><tr ng-if=\"loading\"><td class=\"loading-holder\" colspan=\"40\"><div class=\"text-center\"><span class=\"loading\"></span></div></td></tr><tr ng-if=\"total === 0\"><td colspan=\"40\"><div class=\"text-center\">抱歉，没有相关数据</div></td></tr><tr ng-repeat=\"item in list\" ng-if=\"!loading\"><td>{{ item.busId }}</td><td>{{ item.busName }}</td><td>{{ item.busDesc }}</td><td>{{ item.busLogo }}</td><td>{{ item.busImage }}</td><td>{{ item.content' }}</td><td>{{ item.busHref }}</td><td class=\"text-right\"><a href=\"\" ng-click=\"goDetail(item.busId)\">编辑</a></td></tr></tbody></table><div class=\"text-right\" ng-show=\"total\"><ul class=\"pagination-sm\" pagination ng-model=\"page\" items-per-page=\"size\" total-items=\"total\" ng-change=\"search()\"></ul></div></div>");
}]);

angular.module("modules/red/templates/login.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/red/templates/login.html",
