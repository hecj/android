...
brokerFrontModule.config(['$stateProvider', '$urlRouterProvider',
  function ($stateProvider, $urlRouterProvider) {
    ...
    $urlRouterProvider.otherwise("/brokerfront/login/bcode/136");
...
brokerFrontModule.controller('publicLoginController', ['$scope', '$state', '$stateParams', 'brokerFrontService', 'cookieService', 'msgService', '$timeout',
  function($scope, $state, $stateParams, service, cookieService, message, $timeout) {
...
/*多美理财师邀请列表*/
brokerFrontModule.controller('brokerInviteListController', ['$scope', '$state', 'brokerFrontService', 'timeService',
...
brokerFrontModule.controller('brokerInvestListController', ['$scope', '$state', 'brokerFrontService', 'timeService',
  function($scope, $state, service, timeService) {
    ...
    $scope.investList = function() {
      ...
      params["size"] = size;
      params["mobile"] = $scope.entity.mobile;
      params["InvestTimeBegin"] = ($scope.entity.InvestTimeBegin) ? timeService.getTime($scope.entity.InvestTimeBegin) : null;
      params["InvestTimeEnd"] = ($scope.entity.InvestTimeEnd) ? timeService.getEndTime($scope.entity.InvestTimeEnd) : null;
...
brokerFrontModule.controller('offlineRecieveController', ['$scope', '$state', 'brokerFrontService', 'msgService',
  function($scope, $state, service, message) {
    ...
    $scope.seletAll = function() {
      ...
      for (item in $scope.list) {
        if ($scope.list[item].commissionStatus == 2) {
...
/*提现*/
brokerFrontModule.controller('publicWithdrawController', ['$scope', '$state', 'brokerFrontService',
  function($scope, $state, service) {
    service.myAccount().then(function(res) {
      $scope.entity = res;
...
brokerFrontModule.factory('timeService',['$timeout',function ($timeout) {
  return {
    ...
    getEndTime:function(time){
      var date=new Date(time);
      return new Date(date.getFullYear()+"/"+(date.getMonth()+1)+"/"+date.getDate())*1+(24*3600-1);
    }
  }
}]);
...
angular.module("modules/brokerfront/templates/broker.invite.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/broker.invite.list.html",
    ...
    "                    <div class=\"col-xs-2\">&nbsp;</div>\n" +
    "                    <div class=\"col-xs-10\">\n" +
    "                        <input type=\"button\" value=\"筛选\" ng-click=\"invitelist()\" class=\"determine floatL\" />\n" +
    "                        <input type=\"button\" value=\"取消\" ng-click=\"windowShow(false)\" class=\"cancel floatR\" />\n" +
    "                    </div>\n" +
    "                  </div>\n" +
    "                </form>              \n" +
...
angular.module("modules/brokerfront/templates/offline.recieve.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/offline.recieve.html",
    ...
    "                        <a ng-if=\"item.commissionStatus==2\" class=\"corgray\">已领</a>\n" +
...
angular.module("modules/brokerfront/templates/public.account.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/public.account.html",
    ...
    "        \n" +
    "        <div class=\"tcaccount\">\n" +
    "        	<div class=\"floatL lpic\"><a class=\"tipnub\" ng-if=\"entity.prRelCount\">{{entity.prRelCount}}</a></div>\n" +
...
angular.module("modules/brokerfront/templates/public.bill.list.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/public.bill.list.html",
    ...
    "                            </td>\n" +
    "                            <td><p ng-class=\"{corred:item.operateType!=1003,corgreen:item.operateType==1003}\">{{item.handleSum|operatefilter:item.operateType}}</p></td>\n" +
...
angular.module("modules/brokerfront/templates/public.login.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/public.login.html",
    ...
    "                        </div>\n" +
    "\n" +
    "                        <div class=\"col-xs-12\" ng-if=\"codeType=='aaacode'\">\n" +
    "                            <label class=\"checkbox-inline regis_tip\">\n" +
    "                              <input type=\"checkbox\" id=\"inlineCheckbox1\" ng-model=\"reg.recieve_2\" value=\"option1\">我已阅读并同意<a href=\"#\">免责条款</a>\n" +
    "                            </label>\n" +
    "                        </div>\n" +
    "                    </div>\n" +
    "                    <div class=\"form-group\">\n" +
    ...
    "            <button type=\"submit\" class=\"btn btn-primary btn-lg btn-block\" ng-click=\"checkRealName()\">完成验证</button>\n" +
    "        </form>\n" +
    "    </div>\n" +
    "\n" +
    "</div>\n" +
    ...
    "                <p class=\"mart2\">验证失败！</p>\n" +
    "                <p class=\"padb1 corgray\">您的认证失败次数超过2次！再次认证需要支付手续费。<span class=\"coryellow\">去平台充值</span></p>\n" +
    "                \n" +
    "                <div class=\"popover_tcon\">\n" +
    ...
    "            </div>\n" +
    "           \n" +
    "           	<button type=\"submit\" class=\"btn btn-primary btn-lg btn-block\">去平台充值</button>\n" +
    "        </div>\n" +
    "    </div>\n" +
    "</div>\n" +
    "\n" +
...
angular.module("modules/brokerfront/templates/public.withdraw.html", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("modules/brokerfront/templates/public.withdraw.html",
    ...
    "            <div class=\"floatL rtxtnub\">\n" +
    "              <div class=\"usrlei\"><span class=\"floatL\">提成账户</span><a class=\"floatL\">?</a></div>\n" +
    ...
    "            <form class=\"form-horizontal\" role=\"form\">\n" +
    "                <div class=\"form-group\">\n" +
    "                    <input type=\"text\" class=\"form-control\" ng-model=\"params.money\" placeholder=\"输入提现金额\">\n" +
    "                </div>\n" +
    ...
    "                </div>\n" +
    "                <button type=\"submit\" class=\"btn btn-primary btn-lg btn-block\" ng-click=\"widthDraw()\">确认提现</button>\n" +
    "            </form>\n" +
    "        </div>\n" +
    "\n" +
    "    </div> \n" +
    ...
    "          </div>\n" +
    "          <button type=\"submit\" class=\"btn btn-primary btn-lg btn-block\" ui-sref=\"brokerfront.account\">我的账户</button>\n" +
    "      </div>\n" +
    "  </div>\n" +
    "\n" +
    "\n" +
